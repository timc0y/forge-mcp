...
export const forgeTools = [
  ...
  { name: 'forge_git_commit', title: 'Commit', description: 'Stage paths and commit as forge-mcp[bot]. Omit message to auto-generate. When auto-push is enabled, pushes the forge/ branch to origin after commit — push failure fails the commit call (work stays local until push succeeds).', inputSchema: { workspace_id: workspaceIdOptional, message: z.string().min(1).max(500).optional(), paths: z.array(z.string().min(1).max(500)).max(100).default([]), expected_revision: revision, idempotency_key: idempotencyOptional }, sideEffect: 'workspace', approval: 'none' },
  ...
  { name: 'forge_submit', title: 'Submit for review', description: 'Auto-commit if needed, push feature branch + stage on a Forge ref, queue draft PR for human approval. Returns a submission_receipt — only echo those fields when claiming work is submitted. Never invent workers.dev URLs.', inputSchema: { workspace_id: workspaceIdOptional, branch: forgeBranch, pr_base: z.string().min(1).max(255).default('main'), base: z.string().min(1).max(255).optional(), title: z.string().min(1).max(256).optional(), body: z.string().max(60000).default(''), task_id: taskId.optional(), idempotency_key: idempotencyOptional }, outputSchema: { submitted: z.boolean(), status: z.string(), deferred_action_id: z.string(), approval_id: z.string(), approval_url: z.string(), staged_ref: z.string(), commit: z.string(), remote_sha: z.string(), branch: z.string(), pr_base: z.string(), comparison_ref: z.string(), files_changed: z.number().int(), auto_committed: z.boolean(), submission_receipt: z.object({ branch: z.string(), remote_sha: z.string(), staged_ref: z.string(), approval_url: z.string(), feature_branch_on_origin: z.literal(true) }), next_step: z.string() }, sideEffect: 'external', approval: 'deferred' },
  { name: 'forge_cloudflare_deploy', title: 'Deploy to Cloudflare', description: 'Deploy with wrangler using an attached Cloudflare vault secret (must include CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID). Pins the account, requires approval, probes the published URL, and returns a deploy_receipt. Do not claim a Worker is live without this receipt.', inputSchema: { workspace_id: workspaceIdOptional, command: z.string().min(1).max(2000).default('npx wrangler deploy'), cwd, expected_url: z.string().url().optional(), approval_id: z.string().startsWith('apr_').optional(), idempotency_key: idempotencyOptional }, outputSchema: { deployed: z.boolean(), account_id: z.string(), worker_name: z.string().nullable(), verified_url: z.string().nullable(), http_status: z.number().int().nullable(), deploy_receipt: z.object({ account_id: z.string(), worker_name: z.string().nullable(), verified_url: z.string().nullable(), http_status: z.number().int().nullable(), command: z.string() }), stdout_tail: z.string(), next_step: z.string() }, sideEffect: 'external', approval: 'required' },
