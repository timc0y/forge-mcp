...
export const forgeTools = [
  ...
  { name: 'forge_operation_get', title: 'Get operation status', description: 'Reconcile an uncertain prior mutation by operation id. Syncs process lifecycle first, then returns whether the operation was accepted, is still active (e.g. managed process running), or completed/failed — without inferring from side effects alone.', inputSchema: { workspace_id: workspaceIdOptional, operation_id: z.string().startsWith('op_').describe('Operation id from a prior mutating tool response.') }, outputSchema: { workspaceId: z.string(), operationId: z.string(), idempotencyKey: z.string(), replayed: z.boolean(), originalOperationId: z.string(), status: z.enum(['accepted', 'active', 'completed', 'failed', 'cancelled']), processId: z.string().nullable(), process: z.unknown().nullable(), dependencyState: dependencyStateOut, workspaceRevision: z.number(), allowedNextActions: z.array(z.string()).optional() }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_operation_reconcile', title: 'Reconcile operation outcome', description: 'Same as forge_operation_get: after a client disconnect or unknown tool outcome, ask whether operation op_... finished, failed, or remains active. Syncs managed-process lifecycle before answering.', inputSchema: { workspace_id: workspaceIdOptional, operation_id: z.string().startsWith('op_').describe('Operation id from a prior mutating tool response.') }, outputSchema: { workspaceId: z.string(), operationId: z.string(), idempotencyKey: z.string(), replayed: z.boolean(), originalOperationId: z.string(), status: z.enum(['accepted', 'active', 'completed', 'failed', 'cancelled']), processId: z.string().nullable(), process: z.unknown().nullable(), dependencyState: dependencyStateOut, workspaceRevision: z.number(), allowedNextActions: z.array(z.string()).optional() }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_process_wait', title: 'Wait for managed process', description: 'Wait server-side for a managed process to reach a terminal state (exit, failure, or cancellation), or time out. Returns the final process record, dependencyState, and whether filesystem changes were committed. Prefer this over polling forge_process_logs. Wait timeouts are observational (process is NOT killed) and return timedOut:true with suggestedTimeoutMs — retry with a larger timeout_ms (use >=600000 for large installs). Do not restart the command.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_').describe('Process id from forge_process_start.'), timeout_ms: z.number().int().min(1000).max(600000).default(120000).describe('Maximum time to wait for the process to finish.') }, outputSchema: { workspaceId: z.string(), process: z.unknown(), dependencyState: dependencyStateOut, timedOut: z.boolean().optional(), suggestedTimeoutMs: z.number().optional(), filesystemCommitted: z.boolean().optional(), finalLogCursor: z.string().nullable().optional(), workspaceRevision: z.number(), allowedNextActions: z.array(z.string()).optional(), next_step: z.string().optional() }, sideEffect: 'none', approval: 'none' },
