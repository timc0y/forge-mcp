...
export const forgeTools = [
  ...
  { name: 'forge_edit', title: 'Edit files', description: 'Write or delete files. The edit is committed straight to GitHub on your branch — there is no separate push, and nothing is ever left only inside the workspace. Set content to null to delete a path. Returns the commit URL.', inputSchema: { workspace_id: workspaceIdOptional, files: z.array(z.object({ path: z.string().min(1).max(1000), content: z.string().max(500000).nullable() })).min(1).max(50), message: z.string().min(1).max(500).optional() }, outputSchema: { mutationOutcome: z.string(), durability: z.string(), on_remote: z.boolean(), durability_statement: z.string(), commit_sha: z.string().optional(), commit_url: z.string().optional(), branch: z.string(), paths: z.array(z.string()), rebased: z.boolean(), next_step: z.string() }, sideEffect: 'external', approval: 'none' },
