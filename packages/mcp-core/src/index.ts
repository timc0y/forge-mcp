...
export const forgeTools = [
  ...
  { name: 'forge_files_write', title: 'Write file', description: 'Create or replace one file. Pass expected_sha256 for conflict-safe overwrite. For many files prefer forge_files_write_batch (folder/file-by-file, small receipts).', inputSchema: { workspace_id: workspaceIdOptional, path: z.string().startsWith('/workspace/repo/').max(1000), content: z.string().max(4000000), expected_sha256: z.string().regex(/^[a-f0-9]{64}$/).optional(), expected_revision: revision, idempotency_key: idempotencyOptional }, sideEffect: 'workspace', approval: 'none' },
  { name: 'forge_files_write_batch', title: 'Write files (batch)', description: 'Write up to 20 files in one call (folder/file-by-file). Returns compact receipts (path + sha) — not file contents. Prefer this over dumping many forge_files_write calls when hosts truncate.', inputSchema: { workspace_id: workspaceIdOptional, files: z.array(z.object({ path: z.string().startsWith('/workspace/repo/').max(1000), content: z.string().max(500000), expected_sha256: z.string().regex(/^[a-f0-9]{64}$/).optional() })).min(1).max(20), expected_revision: revision, idempotency_key: idempotencyOptional }, outputSchema: { written: z.number().int(), files: z.array(z.object({ path: z.string(), resultingSha256: z.string(), sizeBytes: z.number().int() })), workspaceRevision: z.number().int(), next_step: z.string() }, sideEffect: 'workspace', approval: 'none' },
  { name: 'forge_files_patch', title: 'Patch file', description: 'Apply a unified diff (all-or-nothing). Optional paths= keep only those file sections — use for file-by-file / folder-scoped patches when the full diff is huge. On reject, re-read and rewrite with forge_files_write.', inputSchema: { workspace_id: workspaceIdOptional, patch: z.string().min(1).max(2000000), paths: z.array(z.string().min(1).max(500)).max(50).optional(), expected_revision: revision, idempotency_key: idempotencyOptional }, sideEffect: 'workspace', approval: 'none' },
