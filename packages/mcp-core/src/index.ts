...
export const forgeTools = [
  ...
  { name: 'forge_workspace_get', title: 'Get a workspace', description: 'Get a workspace\'s lifecycle state, repository, revision, processes and previews. Read-only and container-free — poll it after forge_workspace_create until state is \'ready\'.', inputSchema: { workspace_id: workspaceIdOptional, compact: z.boolean().optional().describe('Return a token-efficient compact summary.') }, outputSchema: workspaceGetOutput, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_process_wait', title: 'Wait for managed process', description: 'Wait for a managed process to reach a terminal state (exit, failure, or cancellation), or time out. Returns the final process record. This is the safe way to wait for a long-running command that may outlive the chat transport deadline — the process continues under Forge supervision even if the client disconnects. Wait timeouts are observational: the process is not killed; retry with a larger timeout_ms.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_').describe('Process id from forge_process_start.'), timeout_ms: z.number().int().min(1000).max(600000).default(120000).describe('Maximum time to wait for the process to finish.') }, outputSchema: { workspaceId: z.string(), process: z.unknown(), dependencyState: z.object({ lockfileHash: z.string(), installedAt: z.string(), usable: z.boolean() }).nullable().optional(), workspaceRevision: z.number() }, sideEffect: 'none', approval: 'none' },
