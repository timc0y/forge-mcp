...
export const forgeTools = [
  ...
  { name: 'forge_workspace_reconcile', title: 'Reconcile workspace recovery state', description: 'Read the checked-out Git state, reconcile durable metadata, and report whether the current Forge commit has been pushed.', inputSchema: { workspace_id: workspaceIdOptional }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_process_get', title: 'Get managed process status', description: 'Read the actual managed process status, command, working directory and PID for one workspace-owned process.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_') }, sideEffect: 'none', approval: 'none' },
