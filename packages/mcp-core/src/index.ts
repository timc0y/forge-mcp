...
export const forgeTools = [
  ...
  { name: 'forge_deploy', title: 'Deploy', description: 'Deploy from attached vault env names. Picks Cloudflare Wrangler when a token+account pair is present (CLOUDFLARE_* or CF_* aliases). Requires idempotency_key; slow runs return process_id, then the same key yields a verified receipt.', inputSchema: { workspace: addrWorkspace(), workflow: z.enum(['auto', 'cloudflare_wrangler']).default('auto').describe('auto = choose from attached env names; cloudflare_wrangler forces Wrangler.'), command: z.string().min(1).max(2000).optional().describe('Override the workflow default command (e.g. npx wrangler deploy).'), cwd, expected_url: z.string().url().optional(), include_output: z.boolean().default(false), approval_id: z.string().startsWith('apr_').optional(), idempotency_key: idempotency() }, outputSchema: { deployed: z.boolean(), accepted: z.boolean(), workflow: z.string().optional(), process_id: z.string(), approval_id: z.string().optional(), deploy_receipt: z.object({ workflow: z.string(), account_id: z.string(), worker_name: z.string().nullable(), verified_url: z.string().nullable(), http_status: z.number().int().nullable(), command: z.string(), matched_env: z.record(z.string(), z.string()).optional() }).optional(), output_artifact_id: z.string().optional(), next_step: z.string() }, sideEffect: 'external', approval: 'required' },
  { name: 'forge_cloudflare_deploy', title: 'Deploy to Cloudflare', description: 'Alias of forge_deploy forced to Cloudflare Wrangler. Prefer forge_deploy. Accepts CLOUDFLARE_* or CF_* aliases.', inputSchema: { workspace: addrWorkspace(), command: z.string().min(1).max(2000).default('npx wrangler deploy'), cwd, expected_url: z.string().url().optional(), include_output: z.boolean().default(false), approval_id: z.string().startsWith('apr_').optional(), idempotency_key: idempotency() }, outputSchema: { deployed: z.boolean(), accepted: z.boolean(), workflow: z.string().optional(), process_id: z.string(), approval_id: z.string().optional(), deploy_receipt: z.object({ workflow: z.string(), account_id: z.string(), worker_name: z.string().nullable(), verified_url: z.string().nullable(), http_status: z.number().int().nullable(), command: z.string(), matched_env: z.record(z.string(), z.string()).optional() }).optional(), output_artifact_id: z.string().optional(), next_step: z.string() }, sideEffect: 'external', approval: 'required' },
