...
export const forgeTools = [
  ...
  { name: 'forge_review', title: 'Review a site with Parallax', description: 'The easiest and cheapest Forge path. Capture screenshot and accessibility evidence from an existing URL without starting a container, ready for a strict Parallax review.', inputSchema: { url: z.string().url(), captures: z.array(z.object({ selection: z.string().min(1).max(200), path: z.string().startsWith('/'), state: z.string().min(1).max(100).default('entry') })).min(1).max(10), viewports: z.array(z.object({ id: z.string().min(1).max(40), width: z.number().int().min(240).max(1920), height: z.number().int().min(320).max(2160) })).min(1).max(2).default([{ id: 'phone', width: 390, height: 844 }, { id: 'desktop', width: 1440, height: 900 }]), full_page: z.boolean().default(false) }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_shell_exec', title: 'Execute bounded command', description: 'Execute a foreground command in an explicit directory with timeout, output and network-policy bounds. Risky commands return a real user approval URL.', inputSchema: { workspace_id: workspaceId, command: z.string().min(1).max(16384), cwd, timeout_ms: z.number().int().min(100).max(900000).default(300000), environment: z.record(z.string(), z.string()).default({}), network_policy: z.enum(['deny_all','package_install','development','custom_allowlist','unrestricted_with_approval']).default('development'), output_limit_bytes: z.number().int().min(1000).max(1000000).default(200000), expected_revision: revision, idempotency_key: idempotency, approval_id: z.string().startsWith('apr_').optional() }, sideEffect: 'workspace', approval: 'policy' },
  ...
  { name: 'forge_browser_act', title: 'Run bounded browser interactions', description: 'Drive a real user journey against a Forge preview: navigate, click, fill, press keys and wait for selectors or text, then capture a screenshot and accessibility tree of the resulting state. Use this to prove multi-step flows (add to cart, submit a form, open a menu) that a single screenshot cannot.', inputSchema: { workspace_id: workspaceId, preview_id: z.string().startsWith('prv_'), path: z.string().startsWith('/').default('/'), steps: z.array(z.object({ kind: z.enum(['navigate','click','fill','press','wait_for_selector','wait_for_text','wait','reload']), selector: z.string().min(1).max(1000).optional(), value: z.string().max(10000).optional(), key: z.string().min(1).max(40).optional(), text: z.string().min(1).max(1000).optional(), path: z.string().startsWith('/').optional(), timeout_ms: z.number().int().min(100).max(30000).optional() })).min(1).max(20), viewport: z.object({ width: z.number().int().min(240).max(3840), height: z.number().int().min(240).max(2160) }), full_page: z.boolean().default(false) }, sideEffect: 'workspace', approval: 'none' },
