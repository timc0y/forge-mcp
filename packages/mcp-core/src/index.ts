...
export const forgeTools = [
  ...
  { name: 'forge_task_list', title: 'List tasks', description: 'List recent tasks for the account, most recently updated first, optionally filtered by state. Container-free.', inputSchema: { state: z.enum(['planning','ready','coding','validating','previewing','reviewing','awaiting-approval','complete','failed','cancelled']).optional().describe('Only return tasks in this state.'), limit: z.number().int().min(1).max(100).default(20).describe('Maximum number of tasks to return.') }, outputSchema: { tasks: z.array(z.object(taskSummaryOutput)) }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_task_finish', title: 'Finish a task', description: 'Move a task to a terminal state (complete, failed or cancelled). The record and its evidence stay retrievable afterwards. Container-free.', inputSchema: { task_id: taskId, outcome: z.enum(['complete','failed','cancelled']).describe('The terminal state to move the task to.'), note: z.string().max(2000).optional().describe('Optional closing note added to the task.'), expected_revision: revision }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_workspace_destroy', title: 'Destroy a workspace', description: 'Destroy a workspace: revoke previews and capabilities, stop processes and tear down the container. Do this once the task or review is complete.', inputSchema: { workspace_id: workspaceId, preserve_artifacts: z.boolean().default(true).describe('Keep captured artifacts after teardown.'), expected_revision: revision, idempotency_key: idempotency }, sideEffect: 'destructive', approval: 'policy' }
