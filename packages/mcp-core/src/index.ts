...
const workspaceGetOutput = {
  ...
  gitIntegrity: z.object({
    ...
    gitHeadReadable: z.boolean().describe('Whether .git/HEAD is readable.'),
    gitIndexReadable: z.boolean().describe('Whether the Git index is readable.'),
    workingTreeReadable: z.boolean().describe('Whether the working tree is readable.'),
    recommendedAction: z.string().describe('What to do next.'),
    destructiveRecoveryRequired: z.boolean().describe('Whether a destructive checkpoint restore is needed.')
...
export const forgeTools = [
  ...
  { name: 'forge_diff_metadata', title: 'Summarize the outgoing diff', description: 'Summarize the outgoing diff into deterministic, compact metadata: changed files, additions/deletions, changed exports, tests, config, migrations, possible secret exposure, risk areas and the files worth reading first, plus targeted verification suggestions. Syntax-only and cheap — inspect the raw diff with forge_git_outgoing_diff before any Git mutation.', inputSchema: { workspace_id: workspaceIdOptional, base: z.string().min(1).max(255).default('main').describe('Base branch to diff the current branch against.') }, outputSchema: diffMetadataOutput, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_shell_exec', title: 'Run a command', description: 'Run a foreground command in the workspace with timeout, output and network-policy bounds. Risky commands return a real user approval URL; idempotency_key must be unique per distinct command.', inputSchema: { workspace_id: workspaceIdOptional, command: z.string().min(1).max(16384).describe('Command to run.'), cwd, timeout_ms: z.number().int().min(100).max(900000).default(300000).describe('Kill the command after this many milliseconds.'), environment: z.record(z.string(), z.string()).default({}).describe('Extra environment variables.'), network_policy: z.enum(['deny_all','package_install','development','custom_allowlist','unrestricted_with_approval']).default('development').describe('Outbound network policy for this command.'), output_limit_bytes: z.number().int().min(1000).max(1000000).default(200000).describe('Truncate captured output past this many bytes.'), expected_revision: revision, idempotency_key: idempotencyOptional, approval_id: z.string().startsWith('apr_').optional().describe('Approval id returned by a prior blocked call.') }, sideEffect: 'workspace', approval: 'policy' },
  ...
  { name: 'forge_process_get', title: 'Get managed process status', description: 'Read the actual managed process status, command, working directory and PID for one workspace-owned process.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_') }, outputSchema: { workspaceId: z.string(), process: z.unknown(), recorded: z.unknown().nullable(), workspaceRevision: z.number() }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_process_stop', title: 'Stop managed process', description: 'Stop one explicit workspace-owned managed process. Forge never guesses by port or process name.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_'), expected_revision: revision, idempotency_key: idempotencyOptional }, sideEffect: 'workspace', approval: 'none' },
  { name: 'forge_process_wait', title: 'Wait for managed process', description: 'Wait for a managed process to reach a terminal state (exit, failure, or cancellation), or time out. Returns the final process record. This is the safe way to wait for a long-running command that may outlive the chat transport deadline — the process continues under Forge supervision even if the client disconnects.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_').describe('Process id from forge_process_start.'), timeout_ms: z.number().int().min(1000).max(600000).default(120000).describe('Maximum time to wait for the process to finish.') }, outputSchema: { workspaceId: z.string(), process: z.unknown(), workspaceRevision: z.number() }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_process_cancel', title: 'Cancel managed process', description: 'Cancel (SIGTERM then SIGKILL) one explicit workspace-owned managed process. The process is marked as cancelled and removed from the workspace process table. Use this instead of forge_process_stop when you want to signal that the process was intentionally aborted rather than gracefully stopped.', inputSchema: { workspace_id: workspaceIdOptional, process_id: z.string().startsWith('proc_'), expected_revision: revision, idempotency_key: idempotencyOptional }, sideEffect: 'workspace', approval: 'none' },
