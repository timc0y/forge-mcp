...
const deployProfileId = z.string().regex(/^dpf_[0-9a-hjkmnp-tv-z]{20,32}$/).describe('Deploy profile id (dpf_...).');
...
export const forgeTools = [
  ...
  { name: 'forge_observer_workspaces', title: 'Observer: list workspaces', description: 'Read-only snapshot of live workspace slots, tasks, and branch state. Includes lifecycle/executor_state — state:requested with empty processes is healthy lazy create (use forge_files_read/forge_edit), not a hung provisioner. Repeated identical polls attach stop_polling guidance.', inputSchema: {}, sideEffect: 'none', approval: 'none' },
  { name: 'forge_observer_workspace', title: 'Observer: workspace detail', description: 'Read-only observer bundle: processes, merged MCP tool trail, log tail, plus lifecycle guidance. Empty processes/logs while state is requested are expected. Do not poll this tool waiting for requested→ready; that transition happens only after an execution tool starts the executor.', inputSchema: { workspace: addrWorkspace() }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_merge', title: 'Merge for review', description: 'Ask the human to merge your work. Opens a pull request from your branch and returns one approval link — echo the link, do not invent URLs. Your branch is already on GitHub; nothing is pushed here. Do not call again while the same submission is pending.', inputSchema: { workspace: addrWorkspace(), pr_base: z.string().min(1).max(255).default('main'), title: z.string().min(1).max(256).optional(), body: z.string().max(60000).default(''), task_id: taskId.optional(), idempotency_key: idempotencyOptional() }, outputSchema: { submitted: z.boolean(), replayed: z.boolean().optional(), submission_receipt: z.object({ branch: z.string(), remote_sha: z.string().optional(), staged_ref: z.string(), approval_id: z.string(), approval_url: z.string(), files_changed: z.number().int(), feature_branch_on_origin: z.boolean() }), next_step: z.string() }, sideEffect: 'external', approval: 'deferred' },
  { name: 'forge_deploy_profiles', title: 'List deploy profiles', description: 'List approved deploy profiles for a repository/workspace. Use before guessing a Cloudflare deploy command.', inputSchema: { workspace: addrWorkspace() }, outputSchema: { profiles: z.array(z.unknown()), returned: z.number().int(), next_step: z.string(), allowedNextActions: z.array(z.string()).optional() }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_deploy_profile_plan', title: 'Plan deploy profile', description: 'Inspect repo deploy hints (wrangler.toml/package scripts) and propose a Cloudflare deploy profile. Repo files are hints only; approval saves the authoritative Forge profile.', inputSchema: { workspace: addrWorkspace(), environment: deployEnvironment.optional() }, outputSchema: { state: z.enum(['ready_to_approve', 'needs_user_choice', 'blocked']), draft: z.unknown().optional(), candidates: z.array(z.unknown()).optional(), detected: z.record(z.string(), z.unknown()), profile_hash: z.string().optional(), next_step: z.string(), allowedNextActions: z.array(z.string()) }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_deploy_profile_approve', title: 'Approve deploy profile', description: 'Request approval to save or update a deploy profile snapshot. Profile approval is distinct from deploy approval.', inputSchema: { workspace: addrWorkspace(), label: z.string().min(1).max(100), environment: deployEnvironment.default('preview'), workflow: z.enum(['cloudflare_wrangler']).default('cloudflare_wrangler'), cwd, command: z.string().min(1).max(2000), expected_url: z.string().url().optional(), expected_worker_name: z.string().min(1).max(200).optional(), account_id: z.string().min(1).max(200).optional(), map_env: z.record(z.string().min(1).max(100), z.string().min(1).max(100)).default({ CLOUDFLARE_API_TOKEN: 'CLOUDFLARE_API_TOKEN' }), source_hint: z.string().min(1).max(500).default('agent-provided'), approval_id: z.string().startsWith('apr_').optional(), idempotency_key: idempotencyOptional() }, outputSchema: { accepted: z.boolean(), approval_id: z.string().optional(), profile: z.unknown().optional(), profile_hash: z.string().optional(), next_step: z.string().optional(), allowedNextActions: z.array(z.string()).optional() }, sideEffect: 'external', approval: 'required' },
  { name: 'forge_deploy', title: 'Deploy', description: 'Approved deploy using attached vault secrets or an approved deploy profile. Prefer profile_id from forge_deploy_profiles; otherwise pass command/cwd/map_env directly. Requires idempotency_key; slow runs return process_id, then the same key yields a verified receipt.', inputSchema: { workspace: addrWorkspace(), profile_id: deployProfileId.optional(), workflow: z.enum(['auto', 'cloudflare_wrangler']).default('auto'), command: z.string().min(1).max(2000).optional().describe('Override default command (e.g. npx wrangler deploy).'), map_env: z.record(z.string().min(1).max(100), z.string().min(1).max(100)).optional().describe('Process env name → attached vault var name. Example: { "CLOUDFLARE_API_TOKEN": "CF_KEY" }.'), cwd, expected_url: z.string().url().optional(), include_output: z.boolean().default(false), approval_id: z.string().startsWith('apr_').optional(), idempotency_key: idempotency() }, outputSchema: { deployed: z.boolean(), accepted: z.boolean(), workflow: z.string().optional(), process_id: z.string(), approval_id: z.string().optional(), deploy_receipt: z.object({ workflow: z.string(), account_id: z.string().nullable(), worker_name: z.string().nullable(), verified_url: z.string().nullable(), http_status: z.number().int().nullable(), command: z.string(), map_env: z.record(z.string(), z.string()).optional(), profile_id: z.string().optional(), profile_label: z.string().optional() }).optional(), output_artifact_id: z.string().optional(), next_step: z.string() }, sideEffect: 'external', approval: 'required' },
  ...
  { name: 'forge_secret_accounts', title: 'List provider accounts', description: 'Use a stored API token to list Cloudflare accounts (id + name only). Pass token_var when the token is not named CLOUDFLARE_API_TOKEN (e.g. CF_KEY). Ask the user which account_id to keep, then forge_secret_update to set CLOUDFLARE_ACCOUNT_ID (env merges; token stays).', inputSchema: { secret_id: secretId, token_var: z.string().min(1).max(100).optional().describe('Vault env name holding the API token. Defaults to CLOUDFLARE_API_TOKEN, or the sole var on the secret.') }, outputSchema: { secret_id: z.string(), token_var: z.string(), accounts: z.array(z.object({ id: z.string(), name: z.string() })), returned: z.number().int(), next_step: z.string() }, sideEffect: 'none', approval: 'none' },
  ...
  { name: 'forge_secret_update', title: 'Update secret', description: 'Update label/provider, or patch env vars (merged into existing — so you can add CLOUDFLARE_ACCOUNT_ID without re-sending the token). Optional unset_env removes keys.', inputSchema: { secret_id: secretId, label: z.string().min(1).max(100).optional(), provider: secretProvider.optional(), env: secretEnv.optional(), unset_env: z.array(z.string().min(1).max(100)).max(50).optional() }, sideEffect: 'external', approval: 'none' },
