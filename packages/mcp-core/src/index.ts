...
export const forgeTools = [
  ...
  { name: 'forge_submit', title: 'Submit for review', description: 'Auto-commit if needed, stage on a Forge ref, queue draft PR for human approval. Outgoing diff uses workspace requestedRef; pr_base (default main) is the merge target only.', inputSchema: { workspace_id: workspaceIdOptional, branch: forgeBranch, pr_base: z.string().min(1).max(255).default('main'), base: z.string().min(1).max(255).optional(), title: z.string().min(1).max(256).optional(), body: z.string().max(60000).default(''), task_id: taskId.optional(), idempotency_key: idempotencyOptional }, outputSchema: { submitted: z.boolean(), status: z.string(), deferred_action_id: z.string(), approval_id: z.string(), approval_url: z.string(), staged_ref: z.string(), commit: z.string(), branch: z.string(), pr_base: z.string(), comparison_ref: z.string(), files_changed: z.number().int(), auto_committed: z.boolean(), next_step: z.string() }, sideEffect: 'external', approval: 'deferred' },
