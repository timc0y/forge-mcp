...
export const forgeTools = [
  ...
  // Files
  { name: 'forge_files_list', title: 'List files', description: 'List a bounded file tree under /workspace.', inputSchema: { workspace_id: wsid(), path: z.string().startsWith('/workspace').default('/workspace/repo'), depth: z.number().int().min(1).max(20).default(4), limit: z.number().int().min(1).max(10000).default(1000) }, outputSchema: { entries: z.array(z.unknown()), truncated: z.boolean(), hint: z.string().optional() }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_files_read', title: 'Read files', description: 'Read one file (path) or several (paths) with content hashes for conflict-safe edits.', inputSchema: { workspace_id: wsid(), path: z.string().startsWith('/workspace').optional(), paths: z.array(z.string().startsWith('/workspace')).min(1).max(20).optional(), start_line: z.number().int().positive().optional(), end_line: z.number().int().positive().optional(), max_bytes: z.number().int().min(1).max(500000).default(200000), compact: z.boolean().optional() }, outputSchema: filesReadOutput, sideEffect: 'none', approval: 'none' },
