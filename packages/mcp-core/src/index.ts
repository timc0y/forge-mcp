...
const addrWorkspace = () => z.string().min(1).max(400).optional()
  .describe('"owner/repo#branch", or just the branch. Omit to use your one open workspace.');
...
export const forgeTools = [
  ...
  { name: 'forge_process_wait', title: 'Wait for process', description: 'Wait for a managed process to finish, or time out. Timeouts are observational (process is NOT killed) and return timedOut:true with suggestedTimeoutMs — retry with a larger timeout_ms (use >=600000 for large installs). Do not restart the command.', inputSchema: { workspace: addrWorkspace(), process_id: z.string().startsWith('proc_'), timeout_ms: z.number().int().min(1000).max(600000).default(120000) }, outputSchema: { workspaceId: z.string(), process: z.unknown(), dependencyState: dependencyStateOut, timedOut: z.boolean().optional(), suggestedTimeoutMs: z.number().optional(), filesystemCommitted: z.boolean().optional(), finalLogCursor: z.string().nullable().optional(), workspaceRevision: z.number(), allowedNextActions: z.array(z.string()).optional(), next_step: z.string().optional() }, sideEffect: 'none', approval: 'none' },
