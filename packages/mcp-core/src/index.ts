...
const credentialProfileId = z.string().regex(/^crp_[0-9a-hjkmnp-tv-z]{20,32}$/);
const credentialMetadata = z.object({ account_id: z.string().regex(/^[a-f0-9]{32}$/i).optional() }).default({});
...
export const forgeTools = [
  ...
  { name: 'forge_workspace_reconcile', title: 'Reconcile workspace recovery state', description: 'Read the checked-out Git state, reconcile durable metadata, and report whether the current Forge commit has been pushed.', inputSchema: { workspace_id: workspaceId }, sideEffect: 'none', approval: 'none' },
  { name: 'forge_cloudflare_deploy', title: 'Deploy with Cloudflare Wrangler', description: 'Deploy the workspace with pnpm exec wrangler using the selected Cloudflare profile for this one approved command. The API token is redacted from all results and is never persisted in the sandbox.', inputSchema: { workspace_id: workspaceId, credential_profile_id: credentialProfileId.optional(), environment: z.string().regex(/^[A-Za-z0-9_-]{1,64}$/).optional(), config_path: z.string().startsWith('/workspace/repo/').max(500).refine((value) => !value.split('/').includes('..') && !value.includes('\0')).optional(), approval_id: z.string().startsWith('apr_').optional(), expected_revision: revision, idempotency_key: idempotency }, sideEffect: 'external', approval: 'required' },
