...
export const forgeTools = [
  ...
  { name: 'forge_dependencies_install', title: 'Install workspace dependencies', description: 'Install dependencies as a managed background process using the detected package manager with the exact pinned version. Waits for completion, proves the package tree is shell-visible, records the lockfile hash, creates a completion checkpoint, and reports whether dependencies are usable. Prefer this or forge_process_start+wait over a long foreground shell install.', inputSchema: { workspace_id: workspaceIdOptional, frozen_lockfile: z.boolean().default(true).describe('Use --frozen-lockfile / --immutable when true (fails if the lockfile is out of sync). Set false to allow lockfile updates.'), allow_lockfile_update: z.boolean().default(false).describe('Allow the install to update the lockfile when it is out of sync with the manifest.'), network_policy: z.enum(['deny_all','package_install','development','custom_allowlist','unrestricted_with_approval']).default('package_install').describe('Outbound network policy for this install.'), timeout_ms: z.number().int().min(10000).max(900000).default(600000).describe('Maximum time to wait for the managed install to finish.'), expected_revision: revision, idempotency_key: idempotencyOptional }, outputSchema: { workspaceId: z.string(), success: z.boolean().optional(), exitCode: z.number().optional(), packageManager: z.string().optional(), installCommand: z.string().optional(), lockfileHashBefore: z.string().optional(), lockfileHashAfter: z.string().optional(), lockfileChanged: z.boolean().optional(), dependencyState: dependencyStateOut, processId: z.string().optional(), managedProcess: z.boolean().optional(), filesystemCommitted: z.boolean().optional(), checkpoint: z.object({ snapshotId: z.string(), createdAt: z.string(), providerVersion: z.string() }).optional(), operationId: z.string(), workspaceRevision: z.number(), replayed: z.boolean().optional(), idempotencyKey: z.string().optional(), originalOperationId: z.string().optional(), stderr: z.string().optional(), stdout: z.string().optional(), allowedNextActions: z.array(z.string()).optional() }, sideEffect: 'workspace', approval: 'policy' },
