...
const ALLOWLIST_C: ReadonlyArray<{ file: string; message: string; note: string }> = [
  ...
  { file: 'apps/forge-edge-gateway/src/handlers/execution.ts', message: 'No dev server command was detected for this project, so there is nothing to screenshot. If dependencies are missing call forge_deps_install and forge_process_wait first; otherwise start the server with forge_shell async:true, then call forge_preview again (omit preview_id). Do not open a second workspace. ‖ The repository preview configuration is invalid: ${lastReason} ‖ The preview was not ready inside this call\'s host-safe startup budget (${lastReason}). If node_modules is missing call forge_deps_install then forge_process_wait; otherwise check forge_process_logs and retry forge_preview without restarting an already-running server or opening a second workspace.', note: 'Branching preview startup messages: each branch explains the failure and gives the next public recovery action.' },
