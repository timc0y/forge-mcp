...
const ALLOWLIST_A: ReadonlyArray<{ tool: string; field: string; note: string }> = [
  ...
  { tool: 'forge_workspace_get', field: 'blockingProcessId', note: 'Nested in gitIntegrity. Its value is a process id, consumable by the generic `process_id` input on forge_process_list/forge_process_wait/forge_process_stop — qualified with "blocking" to say which process, not a second concept the name-matcher cannot see.' },
...
const ALLOWLIST_C: ReadonlyArray<{ file: string; message: string; note: string }> = [
  ...
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Outgoing diffs must compare against the workspace requestedRef recorded at creation.', note: '84 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Provide outcome and/or handoff_summary (+ next_steps).', note: '54 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'handoff_summary requires next_steps (1–20 items).', note: '49 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Task is already in the terminal state "${task.state}" and cannot be finished again.', note: '83 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'force requires a note explaining what remains unverified: ${gaps.join(\' \')}', note: '75 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Workspace ${workspaceId} could not be provisioned.${               failure?.message ?  ‖ provision ‖  : \'\'             } This is a provisioning fault, not a repository permission problem, and there is no read-only mode to fall back to. Call forge_workspace_create again; if it fails a second time, report that rather than working around it.', note: 'branching message — branch 1 (86c): no next action; branch 2 (9c): too short; branch 3 (239c): fine.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Provide either path or a non-empty paths array.', note: '47 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: '${file.path}: send either content (whole file, or null to delete) or replace (fragments), not both and not neither.', note: '115 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: '"${duplicate}" appears twice in one edit. Send each path once — the last write would silently win.', note: '98 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: '${file.path} does not exist on ${branch}, so there is nothing to replace in it. Send content to create it.', note: '106 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'GitHub returned HTTP ${listed.status} listing pull requests.', note: '60 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Pull request #${number} was not found in ${owner}/${repo}.', note: '58 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'GitHub returned HTTP ${closed.status} closing #${number}.', note: '57 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'force:true requires a reason, so the record says why a pull request was merged past its own blockers.', note: '101 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'GitHub refused the merge of #${number} with HTTP ${merged.status}. Nothing was merged.', note: '86 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'GitHub returned HTTP ${listed.status} reading history for ${owner}/${repo}. ‖ No history for ${text(input.path)} on ${input.ref === undefined ? \'the default branch\' : text(input.ref)} (HTTP ${listed.status}). Check the path.', note: 'branching message — branch 1 (75c): no next action; branch 2 (146c): no next action.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'GitHub returned HTTP ${listed.status} listing branches for ${owner}/${repo}.', note: '76 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'No merged branches to delete in ${owner}/${repo}. ‖ Branch ${input.branch === undefined ? \'(none given)\' : text(input.branch)} does not exist in ${owner}/${repo}. Run action:\'list\' first.', note: 'branching message — branch 1 (49c): no next action; branch 2 (135c): no next action.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'This workspace has no agent branch to merge. Nothing has been edited yet.', note: '73 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'This workspace is on ${workspaceBranch}, not ${branch}. Omit branch and Forge merges the one you have been editing.', note: '115 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'There is nothing to submit: ${branch} has no changes against ${comparisonRef}. Make the change first, then submit.', note: '114 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'Wrangler deploy failed (exit ${result.exitCode}). Do not claim the Worker is live.', note: '82 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'This url_review artifact belongs to a different project.', note: '56 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/mcp-session.ts', message: 'No artifact with this artifact_id exists in this workspace.', note: '59 chars — explains what went wrong but names no next action or tool.' },
  { file: 'apps/forge-edge-gateway/src/repo-paths.ts', message: '"${path}" is not a path inside the repository.', note: '46 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'apps/forge-edge-gateway/src/workspace-coordinator.ts', message: 'Stop or wait for workspace-owned processes before a filesystem mutation or checkpoint so Forge can capture an immutable recovery snapshot.', note: '138 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Invalid GitHub repository owner or name.', note: '40 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Workspace provisioning timed out.', note: '33 chars — restates the failure; no cause, no next action.' },
  { file: 'packages/application/src/index.ts', message: 'Workspace Git state has no checked-out commit.', note: '46 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Forge cannot recover this workspace because it has no durable active checkpoint.', note: '80 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Forge cannot recover this workspace because its active checkpoint is unavailable.', note: '81 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Repository clone failed.', note: '24 chars — restates the failure; no cause, no next action.' },
  { file: 'packages/application/src/index.ts', message: 'Workspace cannot be provisioned from ${record.workspace.state}.', note: '63 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Workspace provisioning probe failed.', note: '36 chars — restates the failure; no cause, no next action.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Workspace provisioning failed.', note: '30 chars — restates the failure; no cause, no next action.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Workspace mount is missing, but Forge will not restore a checkpoint while managed mutating processes still have uncheckpointed filesystem writes.', note: '145 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'The repository checkout is missing; the workspace has been marked failed. Create a new workspace to continue.', note: '109 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Workspace content changed while Forge was creating a checkpoint; the incomplete checkpoint was not accepted.', note: '108 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'The requested workspace checkpoint is unavailable.', note: '50 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Forge refuses an explicit restore from a legacy checkpoint without a pre-capture filesystem manifest.', note: '101 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'The requested checkpoint filesystem does not match its durable manifest.', note: '72 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'Forge could not restore the requested checkpoint; the previous workspace snapshot was recovered. ‖ Forge could not restore the requested checkpoint or recover the previous workspace snapshot.', note: 'branching message — branch 1 (96c): no next action; branch 2 (92c): no next action.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'The managed process was not found in this workspace.', note: '52 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'No operation with that id is recorded for this workspace.', note: '57 chars — explains what went wrong but names no next action or tool.' },
  { file: 'packages/application/src/index.ts', message: 'The process is not owned by this workspace.', note: '43 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Managed dependency install exited successfully but the installed package tree is not visible to the workspace shell.', note: '116 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Forge could not completely enumerate workspace changes.', note: '55 chars — explains what went wrong but names no next action or tool.' },
  ...
  { file: 'packages/application/src/index.ts', message: 'Forge could not calculate a complete filesystem manifest for the checkpoint.', note: '76 chars — explains what went wrong but names no next action or tool.' },
