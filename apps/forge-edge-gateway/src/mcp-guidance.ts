...
export const FORGE_MCP_INSTRUCTIONS = [
  ...
  'UI / design iteration: implement with forge_edit, then forge_preview (or forge_review for an already-deployed URL). If preview fails for missing deps, forge_deps_install → forge_process_wait first. Inspect every returned screenshot before the next edit. One coherent edit per turn; re-capture only the routes that changed. Omit stale preview_id values.',
