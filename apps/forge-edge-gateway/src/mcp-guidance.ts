...
export const FORGE_MCP_INSTRUCTIONS = [
  'Forge has two planes. GitHub is durable: forge_edit commits straight to your branch and returns commit_url. The optional executor runs commands; files those commands change are disposable until you recreate them with forge_edit. Save/persist/commit always means forge_edit — never shell redirects, sed -i, git add/commit/push, or forge_process_wait.',
  ...
  'Wrong order: never forge_edit/shell/preview/merge before forge_workspace_create. Never forge_workspace_create a second time for the same repository while one is live — reuse owner/repo#branch or forge_observer_workspaces. After forge_merge, do not edit that branch further for the same submission; destroy the workspace. After destroy, forge_workspace_create again if you need more work — do not retry the old workspace_id.',
  ...
  'UI / design iteration: implement with forge_edit, then forge_preview (or forge_review for an already-deployed URL). If preview fails for missing deps, forge_deps_install → forge_process_wait first — never forge_shell or forge_preview while an install is still running. Inspect every returned screenshot before the next edit. One coherent edit per turn; re-capture only the routes that changed. Omit stale preview_id values.',
  'Bug fixes: reproduce with the cheapest evidence (forge_review URL, then forge_files_read, then forge_shell). Change the failing code and its test together in one forge_edit. Re-run the narrow check before broadening. Green tests on the executor alone are not done — require a forge_edit commit_url for code changes.',
  'Executor rules: the first shell/install/preview/deploy allocates compute. If it reports provisioning, call forge_workspace_get; if still provisioning, wait a few seconds and call forge_workspace_get again until ready, then retry the same execution tool — never create a second workspace. forge_process_wait is at most 30s per call — timedOut:true means observe again with the same process_id, never restart or raise the wait. While a deps install is running, only wait/logs/list/stop — do not shell or preview. Truncated forge_files_read must not become a whole-file forge_edit content rewrite. forge_workspace_get defaults to compact. After forge_merge, echo approval_url once — do not poll or re-call forge_merge. Never git add/commit/push. Deploys: forge_cloudflare_deploy → echo deploy_receipt.verified_url only. Repository truth is forge_files_read / forge_diff_metadata — shell cat proves only executor state.',
  'When context is compressed or the session reconnects: forge_task_get mode:resume (or forge_observer_workspaces if addressing is unclear), then continue from next_step. Keep task_id. Do not create a second workspace for the same repository task.'
].join(' ');
