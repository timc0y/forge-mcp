...
export const FORGE_MCP_INSTRUCTIONS = [
  ...
  'Executor: first shell/install/preview allocates compute. If provisioning, poll forge_workspace_get until ready then retry the same tool — never create a second workspace. forge_process_wait ≤30s; timedOut means wait again with the same process_id. While install runs: only wait/logs/list/stop. Truncated forge_files_read must not become whole-file forge_edit. forge_workspace_get defaults to compact. Omit stale preview_id. Deploys: forge_secret_list → attach → forge_deploy (picks workflow from attached env names; Cloudflare accepts CLOUDFLARE_* or CF_* aliases) → echo deploy_receipt.verified_url only.'
].join(' ');
