...
export const FORGE_MCP_INSTRUCTIONS = [
  ...
  'Executor rules: the first shell/install/preview/deploy allocates compute. If a tool reports provisioning, call forge_workspace_get once, then retry. forge_process_wait is at most 30s per call — timedOut:true means observe again with the same process_id, never restart. Never git push. Deploys: forge_cloudflare_deploy → echo deploy_receipt.verified_url only.',
