          ...
          if (input.async === true || escalated) {
            ...
            return {
              ...
              next_step: isWranglerDeployCommand(command)
                ? `Running wrangler in the background as ${processId}. Call forge_process_wait with that process_id; when it finishes, retry forge_deploy with the same idempotency_key/approval_id for deploy_receipt.verified_url — do not invent a URL from logs.`
