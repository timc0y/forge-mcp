...
Forge verifies the requested runtime from inside the provisioned sandbox before a workspace is marked ready. A requested Node profile must report that exact Node major plus Corepack; a mismatch is a provisioning failure, never a misleading ready workspace. The local Docker provider selects `node:22-bookworm-slim` or `node:24-bookworm-slim` from the requested profile. Cloudflare Sandbox is configured with one deployed base image, so Node 24 remains explicit opt-in there until its configured image is upgraded and compatibility-tested.
