...
| --- | --- | --- | --- | --- | --- |
| **Cloudflare Sandbox** | Direct fit: command execution, long-running processes and port access are native SDK concepts ([SDK](https://developers.cloudflare.com/sandbox/), [ports](https://developers.cloudflare.com/sandbox/api/ports/)). | Existing provider has already demonstrated the required lifecycle, but production acceptance currently has an unresolved live round trip. Sandbox state is a cache; the provider's backup APIs are optional under the GitHub-authoritative model ([backups](https://developers.cloudflare.com/sandbox/api/backups/)). | Isolated Containers behind Workers; Forge already has the private Worker bridge. This is the lowest-change path, but the SDK remains pre-1.0 in this repository. | Consumption pricing and account limits are documented by Cloudflare; confirm the current Containers/Sandbox entitlement before launch ([pricing](https://developers.cloudflare.com/containers/pricing/), [limits](https://developers.cloudflare.com/containers/platform/limits/)). | **Keep as primary only after the readiness defect is proved fixed.** |
