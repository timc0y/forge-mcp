...
export interface ChangeFileAreaClassification {
  path: string;
  area: ChangeFileArea;
  confidence: number;
}

export async function classifyChangedFileAreasWithJev(
  env: Env,
  files: Array<{ path: string; status?: string; additions?: number; deletions?: number; patch?: string }>
): Promise<ChangeFileAreaClassification[]> {
  if (!env.TYPESAFE_API_KEY || files.length === 0) return [];
  const bounded = files.slice(0, 12);
  const areas: ChangeFileArea[] = [
    'authentication/security',
    'api/integration',
    'ui/ux',
    'data/schema',
  ...
  bounded.forEach((file, index) => {
    questions[`fileArea_${index}`] = {
      type: 'choice',
      instructions: `Which technical concern best describes the role of changed file ${file.path} in this diff?`,
      criteria: areas
    };
  });
  const resp = await typesafeSystemOne(env.TYPESAFE_API_KEY, env.TYPESAFE_BASE_URL, {
  ...
  return bounded.flatMap((file, index): ChangeFileAreaClassification[] => {
    const answer = resp.answers[`fileArea_${index}`] as JevChoiceAnswer | undefined;
    if (!answer?.choice || !areas.includes(answer.choice as ChangeFileArea)) return [];
    const confidence = answer.confidence || Math.max(0, ...Object.values(answer.distribution ?? {}));
    if (confidence < 0.5) return [];
    return [{ path: file.path, area: answer.choice as ChangeFileArea, confidence }];
  });
}
